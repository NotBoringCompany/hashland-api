...
import { AuthenticateUserDto, SendNotificationDto, BroadcastNotificationDto } from '../dto/notification.dto';
...
export class NotificationGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleAuthentication(client: Socket, payload: AuthenticateUserDto): Promise<void> {
        try {
            // Validate JWT token
            ...
            if (decodedToken.sub !== payload.userId) {
                client.emit('error', { message: 'Authentication failed: User ID mismatch' });
                return;
            }
            ...
            this.logger.error(`Authentication error for socket ${client.id}: ${error.message}`);
            client.emit('error', { message: 'Authentication failed: Invalid token' });
        }
    }

    @SubscribeMessage('sendNotification')
    async handleSendNotification(client: Socket, payload: SendNotificationDto): Promise<void> {
        try {
            // Get the authenticated user ID from the socket
            const senderUserId = this.connectionManager.getUserIdFromSocket(client.id);

            if (!senderUserId) {
                client.emit('error', { message: 'You must be authenticated to send notifications' });
                return;
            }
    ...
    async handleBroadcastNotification(client: Socket, payload: BroadcastNotificationDto): Promise<void> {
        try {
            // Get the authenticated user ID from the socket
            const senderUserId = this.connectionManager.getUserIdFromSocket(client.id);

            if (!senderUserId) {
                client.emit('error', { message: 'You must be authenticated to broadcast notifications' });
                return;
            }
    ...
    handleGetConnectionStats(client: Socket): void {
        try {
            ...
            if (!userId) {
                client.emit('error', { message: 'You must be authenticated to access stats' });
                return;
            }
