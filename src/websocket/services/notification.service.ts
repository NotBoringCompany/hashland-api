...
import { NotificationPayload, NotificationType, UserNotification } from '../notification.interface';
...
export class NotificationService {
    ...
    broadcastToAll(notification: NotificationPayload): void {
        try {
            ...
            this.logger.log(`Broadcast notification sent to all users: ${notification.title}`);
        } catch (error) {
            this.logger.error(`Error broadcasting notification: ${error.message}`, error.stack);
        }
    }
     ...
     */
    sendSystemNotification(userId: string, title: string, message: string, data?: any): boolean {
     ...
     */
    sendDrillingNotification(userId: string, title: string, message: string, data?: any): boolean {
        return this.sendToUser(userId, {
     ...
     */
    broadcastSystemNotification(title: string, message: string, data?: any): void {
        this.broadcastToAll({
            type: NotificationType.SYSTEM,
            title,
            message,
