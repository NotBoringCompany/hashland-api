...
export class SchedulerBridgeService implements OnModuleInit {
    ...
    processCycleUpdate(cycleData: any): void {
        try {
            this.logger.debug(`Processing cycle update: ${JSON.stringify(cycleData)}`);
            ...
            this.logger.error(`Error processing cycle update: ${error.message}`, error.stack);
        }
    }
    ...
    processCycleCompletion(cycleData: any): void {
        try {
            this.logger.log(`Processing cycle completion for cycle: ${cycleData.cycleId}`);
            ...
            if (cycleData.topMiners && Array.isArray(cycleData.topMiners)) {
                ...
                const sortedMiners = [...cycleData.topMiners].sort((a, b) => b.hashMined - a.hashMined);
            ...
            this.logger.error(`Error processing cycle completion: ${error.message}`, error.stack);
        }
    }
    ...
    processSystemMaintenance(maintenanceData: any): void {
        try {
            ...
            this.logger.error(`Error processing system maintenance: ${error.message}`, error.stack);
        }
    }
