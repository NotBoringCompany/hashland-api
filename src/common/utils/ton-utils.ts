import { TON_WEB } from 'src/config/ton.config';
import { AddressType } from 'tonweb';
import {
  BlockchainData,
  TxParsedMessage,
} from '../schemas/blockchain-payment.schema';
...
export const verifyTONTransaction = async (
  ...
  while (attempt < maxRetries) {
    try {
      ...
      const txs = await getTONTransactions(address, 1, txHash);
      ...
      const receiverAddress = getNonBounceableAddress(firstOutMsg.destination);
 ...
 */
export const getTONTransactions = async (
  address: string,
  limit: number = 1,
  txHash: string,
) => {
  return await TON_WEB.provider.getTransactions(address, limit, null, txHash);
};
 ...
 */
export const getNonBounceableAddress = (address: AddressType): string => {
  return new TON_WEB.utils.Address(address)?.toString(true, true, false, false);
};
